...
### Data layer (`api/`)
- **`models.py`** — the normalized dataclasses (`Match`, `Team`, `MatchEvent`, `MatchExtras`, `PenaltyKick`, `Lineup`, …) plus enums (`MatchState`, `EventType`). These are the *only* shapes the UI sees. Live-clock display logic (`Match.clock_display`, which interpolates seconds between per-minute polls) and event identity (`MatchEvent.key`, used to diff polls for goal/card detection) live here. **All times display in US Eastern, labeled generically `ET`** — the module owns `EASTERN`/`ET_LABEL`, `eastern_today()` (the `YYYYMMDD` "today" anchor passed to ESPN, since its default scoreboard skips ahead to the next matchday), and `Match.kickoff_et()` / `et_day_key()` / `et_day_label()` (ESPN's UTC dates → Eastern for display and day-grouping). `MatchExtras.shootout` holds the full kick sequence including misses (`PenaltyKick`), which only the `summary` endpoint reports.
...
### Status line (`line.py`)
Deliberately Textual-free and fast. `pick_match` chooses the single most relevant match (live > favorite > next upcoming > recent); `detect_goal_flash` uses a small JSON cache file (`~/.cache/fifatui/`) to flash `GOAL!` on the poll right after a score rises; pre-match rows show the kickoff time in ET (`Match.kickoff_et()`). A statusline must never crash, so `cli._run_line` swallows all exceptions and prints a neutral placeholder.

### TUI (`tui/`)
- **`app.py`** — `FifaApp` owns the poll loop (`refresh_data` on a `set_interval`) and stores `self.matches`. After each poll it calls `on_data_refresh()` on the current screen (duck-typed). Network errors keep the last data and flag the UI rather than crashing. Registers the `broadcast` theme and pushes `MatchListScreen`. It also owns the **date view state** the match list navigates: `self.date` (an explicit `YYYYMMDD`, or `None` for Eastern today) and `self.upcoming` (a forward date-range fixtures list, `today..+UPCOMING_WINDOW_DAYS`). `shift_date` / `goto_today` / `show_upcoming` change the view and re-fetch; `refresh_data` resolves whichever mode is active into the ESPN `dates=` param. On first load, if today's slate is empty it auto-falls back to `show_upcoming()` instead of showing a blank screen.
- **`screens/`** — `matchlist.py` (list, `↵` opens a match) and `watch.py` (the single-match view). `MatchListScreen` provides the date-navigation keybindings — `n` upcoming, `[`/`]` prev/next day, `t`/`esc` back to today — and groups rows by match state on a single day or by ET calendar day in upcoming mode. `WatchScreen` is the **ConsoleHost**: it implements `current_match` / `cached_extras` / `request_extras` / `pin_metric` / `do_refresh` / `go_back` / `do_quit`, and the console widget calls back into it. It also does new-event detection (diffing `MatchEvent.key` sets across polls) and routes goals → celebration, cards/subs → banners, and PRE/IN/POST changes → kickoff/HT/FT banners; for a shootout match it pulls the `summary` once (`_maybe_fetch_shootout`) to get missed kicks the scoreboard omits.
- **`widgets/`** — `scoreboard.py`, `pixelscore.py` (chunky pixel-font score), `sidepanel.py` + `statbars.py` (pinnable comparison bars), `console.py` (the REPL — command dispatch table in `run_command`), `eventfeed.py`, `banner.py`, `goal_celebration.py`, `shootout.py`, `ticker.py`. `anim.py` (`FrameAnimator`, one per widget) is the shared frame-loop helper — a single Textual timer driving `frame(elapsed)` at a target fps until it returns False, with idempotent stop/restart.
...
- **Times are US Eastern, always.** ESPN's dates are UTC; convert through `models.py`'s `EASTERN`/`Match.kickoff_et()` helpers and label them `ET` — never render a raw UTC time or split the ISO string in the UI. Pass an explicit Eastern date to ESPN (`eastern_today()`), never rely on its server default.
